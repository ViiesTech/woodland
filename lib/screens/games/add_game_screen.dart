...
class _AddGameScreenState extends State<AddGameScreen> {
  ...
  Future<void> _saveGame() async {
    ...
    if (_coverImageFile == null) {
      CustomToast.showError(context, 'Please select a cover image');
      return;
    }

    if (_gameUrlController.text.isEmpty) {
      CustomToast.showError(context, 'Please enter game URL');
      return;
    }
    ...
    try {
      ...
      // Save to Firestore
      CustomToast.showInfo(context, 'Saving game...');
      await GameService.addGame(game);

      if (mounted) {
        CustomToast.showSuccess(context, 'Game added successfully!');
        Navigator.pop(context);
      }
    } catch (e) {
      if (mounted) {
        CustomToast.showError(context, 'Error adding game: $e');
      }
    } finally {
      if (mounted) {
        setState(() {
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              // Save Button
              PrimaryButton(
                title: _isLoading ? 'Saving...' : 'Add Game',
                buttonWidth: double.infinity,
                onTap: _isLoading ? null : _saveGame,
              ),
              32.verticalSpace,
            ],
          ),
